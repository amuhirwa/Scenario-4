...
const ACTIVITIES = {
  ...
  RUNNING: {
    emoji: '<i class="fa-solid fa-person-running fa-fade"></i>',
    color: "#00f3ff",
    label: "RUNNING",
  ...
  PRONE_STILL: {
    emoji: '<i class="fa-solid fa-user-injured fa-beat"></i>',
    color: "#ff003c",
    label: "PRONE/STILL",
    bg: "rgba(255,0,60,.15)",
...
function buildMarkerIcon(state) {
  ...
  const html = `
    <div class="soldier-marker">
      ...
      <div class="marker-label" style="color:${act.color}; border-color:${act.color}">${state.soldier_id}</div>
    </div>`;
